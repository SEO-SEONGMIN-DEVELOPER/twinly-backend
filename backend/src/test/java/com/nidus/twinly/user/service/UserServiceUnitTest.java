...
class UserServiceUnitTest {
    ...
    void users_last_page() {
        ...
        given(userRepository.findIdsAfterCursor(null, 3)).willReturn(List.of(1L, 2L));
    ...
    void users_has_more() {
        ...
        given(userRepository.findIdsAfterCursor(10L, 3)).willReturn(List.of(11L, 12L, 13L));
    ...
    void users_without_limit_uses_default() {
        ...
        given(userRepository.findIdsAfterCursor(null, 501)).willReturn(List.of(1L));

        // when: limit 없이 유저 목록 조회
        userService.users(null, null);

        // then: 기본값 기준으로 리포지토리에 위임
        then(userRepository).should().findIdsAfterCursor(null, 501);
    }
    ...
    void users_empty() {
        ...
        given(userRepository.findIdsAfterCursor(99L, 501)).willReturn(List.of());
