...
- `backend/src/test/java/com/nidus/twinly/auth/integration/AuthIntegrationTest.java` (end-to-end, **17건**으로 확장)

---

# [이번 라운드] springdoc 스펙 기준 통합 커버리지 확장

통합 테스트를 3건 → 17건으로 늘려 auth의 12개 오퍼레이션을 전부 관통시켰다.
그 과정에서 **실행으로 확정된** 운영 코드 버그 1건이 나왔다. (BUG-AUTH-01)

## 테스트 통과 여부

| 구분 | 결과 |
|---|---|
| 단위/슬라이스 (20 + 25 + 4) | **49/49 통과** |
| 통합 (`AuthIntegrationTest`) | **17/17 통과** |

## BUG-AUTH-01 — JWT에 `iat`/`jti`가 없어 같은 초의 토큰이 **완전히 동일**하다

- **심각도**: **high**
- **핵심**: 토큰 payload가 `{sub, type, exp}` 뿐이다. `exp`는 초 단위이므로,
  **같은 유저에게 같은 초에 발급한 토큰은 문자열까지 100% 동일**하다. 실측으로 확인했다.

  ```
  jwtService.generateAuthTokenResult(userId) 를 연속 2회 호출
  → REFRESH_EQUAL=true, ACCESS_EQUAL=true
  ```

- **근거 코드 위치**: `backend/src/main/java/com/nidus/twinly/common/jwt/JwtService.java:45-51`
  ```java
  private String generate(Long userId, JwtType type, Instant expiresAt) {
      return Jwts.builder()
              .subject(userId.toString())
              .claim("type", type.name())
              .expiration(Date.from(expiresAt))   // ← iat/jti 없음. 같은 초면 동일 문자열
              .signWith(key)
              .compact();
  }
  ```

### 파급 1 — 같은 초에 로그인을 2번 하면 **두 번째가 500**이다 (실측 확인)

- **증상**: 동일 `smsVerifiedToken`으로 `POST /api/v1/auth/login`을 연속 호출하면
  1회차 200, **2회차 500**.

  ```
  LOGIN_FIRST_STATUS=200
  LOGIN_SECOND_STATUS=500
  DataIntegrityViolationException:
    Duplicate entry '...' for key 'refresh_tokens.uk_refresh_tokens_token_hash'
  ```

- **원인**: `issueAuthToken`이 매번 `RefreshToken.create(userId, hash(refreshToken), ...)`를 새로 저장하는데,
  토큰 문자열이 같으면 `token_hash`도 같아져 유니크 제약을 위반한다.
- **재현 조건**: 로그인 버튼 더블클릭, 클라이언트 재시도, 네트워크 재전송 — **현실에서 흔한 경로**다.
- **근거 코드 위치**
  - `backend/src/main/java/com/nidus/twinly/auth/service/AuthService.java:366` (`issueAuthToken`의 저장)
  - `backend/src/main/resources/db/migration/V1__init_schema.sql:420` (`uk_refresh_tokens_token_hash`)

### 파급 2 — 발급 직후 재발급하면 **500**이다 (실측 확인)

- **증상**: 리프레시 토큰 발급과 같은 초에 `POST /api/v1/auth/refresh`를 호출하면 500.
- **원인**: `refresh`는 기존 행을 `delete`한 뒤 새 토큰을 `save`한다. 그런데 Hibernate의
  기본 flush 순서는 **INSERT가 DELETE보다 먼저**다. 새 토큰의 해시가 기존 것과 같으면
  DELETE가 실행되기 전에 INSERT가 유니크 제약을 때린다.
- **근거 코드 위치**: `backend/src/main/java/com/nidus/twinly/auth/service/AuthService.java:356-358`
  ```java
  refreshTokenRepository.delete(refreshToken);   // 실제 DELETE는 flush 시, INSERT 뒤에 나간다
  return issueAuthToken(userId);                 // INSERT가 먼저 → 중복 키
  ```

### 파급 3 — 토큰 회전(rotation)이 실질적으로 무효다

- **증상**: 회수한 토큰과 새로 발급한 토큰이 같은 값일 수 있다. 회전의 목적(유출된 토큰 무효화)이
  같은 초 안에서는 성립하지 않는다.
- **심각도**: 보안 관점에서는 이 항목이 가장 무겁다. 다만 실제 재발급은 보통 발급 한참 뒤에 일어나므로
  상시 발생하지는 않는다.

### 제안

`iat`와 `jti`를 넣으면 세 파급이 한 번에 사라진다.
...
private String generate(Long userId, JwtType type, Instant expiresAt) {
    ...
    return Jwts.builder()
            .subject(userId.toString())
            .claim("type", type.name())
            .id(UUID.randomUUID().toString())   // jti — 매 발급마다 유일
...
`jti`만 넣어도 토큰이 매번 달라지므로 중복 키 500은 사라진다. `iat`는 감사/디버깅용으로 함께 두면 좋다.
다만 `refresh`의 delete→insert 순서 의존은 별개 문제이니, 회전 로직에서 `saveAndFlush` 또는
삭제를 먼저 flush 하는 방식으로 순서를 명시해 두는 편이 안전하다.

### 통합 테스트에서의 대응

- `refresh_end_to_end`는 **현실 시나리오(발급 한참 뒤 재발급)**를 재현하기 위해 `Thread.sleep(1_100)`을 넣고
  그 이유를 주석으로 남겼다. 버그를 우회한 것이 아니라, 정상 경로를 검증하기 위한 조치다.
- 위 파급 1·2는 조사용 일회성 테스트로 실측 확인 후 제거했다. 수치는 이 문서에 남긴 값 그대로다.
