...
### 1. 세션 발급에 서비스 계층 트랜잭션 경계가 없다

- **증상**: `AnonService.start()`에 `@Transactional`이 없다. 현재는 `anonSessionRepository.save()` 한 번뿐이라
  `SimpleJpaRepository`의 자체 트랜잭션으로 커밋되어 동작에 문제가 없다. 다만 "익명 세션 생성"이라는 유스케이스에
  부수 작업(예: 기본 동의 이력 기록, 기본 페르소나 초기화)이 하나라도 추가되는 순간 원자성이 깨져
  세션만 만들어지고 부수 데이터는 누락되는 부분 성공 상태가 생긴다.
- **재현 조건**: 지금은 재현되지 않음(단일 쓰기). `start()`에 두 번째 쓰기가 추가되고 그 쓰기가 실패할 때 재현.
- **근거 코드 위치**: `backend/src/main/java/com/nidus/twinly/anon/service/AnonService.java:25`
- **심각도**: low
- **제안**: 지금 당장 필요한 조치는 아니다. `start()`에 쓰기 작업이 2개 이상 되는 시점에
  `@Transactional`을 붙여 유스케이스 단위 경계를 명시하면 된다.

### 2. 인증이 없는 공개 쓰기 엔드포인트인데 남용 제어가 없다
...
### 3. 리소스를 생성하는 POST인데 200 OK를 반환한다
...
### 4. (참고) 공통 익명 인증 경로 `AnonService.resolveByToken`의 만료 경계
