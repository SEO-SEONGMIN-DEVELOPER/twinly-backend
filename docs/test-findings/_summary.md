...
기록된 발견사항은 모두 처리되었거나 판단으로 닫혔다. 남은 것은 아래 **다음 개발 항목**뿐이며,
그것들은 고칠 코드가 있는 게 아니라 새로 쓰거나 정책을 정해야 하는 일이다.
...
**원인과 수정 방법이 같은 것끼리 묶어** 한 번에 처리한다. B15와 B8 잔여는 개발 항목으로 옮겼다. **묶음과 개별 항목이 모두 닫혔다.**
...
| people | 9 | **처리 완료(7차).** 목록 조회가 이미 쓰던 조인 쿼리(`findAllByUserIdAndWithPartnerUserIdAndDateIn`)를 상세 조회도 쓰도록 바꿔 필터링을 SQL로 내렸다. 무관한 파트너까지 `findAllById`를 돌던 것도 함께 사라졌다 |
...
| push | 1 | **덮어쓰기로 결론(7차).** `(userId, deviceId)`로 좁히는 안을 검토했으나 되돌렸다. 좁히면 계정 전환 시 두 행이 같은 토큰을 갖게 되고, 그걸 정리하려면 토큰 기준으로 남의 행을 비워야 해 **공격 열쇠가 `deviceId`에서 `fcmToken`으로 옮겨갈 뿐**이다. 대신 소유자를 덮어쓰고 토큰 정리를 같은 유저 범위로 한정했다. 남는 위험은 `deviceId`를 아는 사람이 남의 푸시를 끊는 것(DoS)이며, 알림을 가로채지는 못한다 |
...
| push | 3 | **처리 완료(7차).** 처음에는 소유자 검사(`DEVICE_OWNED_BY_OTHER_USER`) 때문에 upsert로 표현할 수 없어 남겼다. 그 검사가 같은 폰에서 계정을 전환한 사용자의 등록을 영구히 막는 것이 확인되어 덮어쓰기로 방향을 바꿨고, 조건이 없어지면서 다른 셋과 같은 `ON DUPLICATE KEY UPDATE`로 정리됐다 |
...
**마지막으로 남아 있던 2건도 닫았다.** 둘 다 묶음에서 "지금은 미룬다"로 남긴 것이었는데, 해결 방법과
선례가 이미 확정돼 있어 착수 비용이 낮았다.

| 내용 | 처리 |
|---|---|
| 이벤트 상세가 그날 씬 전부를 로드해 메모리에서 상대로 필터링 (구 people #9) | 목록 조회가 이미 쓰던 조인 쿼리를 상세도 쓰게 했다. 무관한 파트너까지 `findAllById`를 돌던 것도 함께 사라졌고, 쿼리에 `ORDER BY`가 있어 `version`을 뽑는 첫 씬도 결정적이 된다 |
| 기기 동시 등록이 유니크 위반으로 500 (구 push #3) | B12의 나머지 셋과 같은 `INSERT ... ON DUPLICATE KEY UPDATE`. 통합 테스트 5건이 그대로 통과해 동작이 바뀌지 않았음을 확인했다 |
