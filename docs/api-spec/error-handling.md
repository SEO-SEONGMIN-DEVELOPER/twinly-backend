...
| 404 | `NOT_FOUND` | **존재하지 않는 경로**로 요청 (인증된 요청 기준) | 라우팅 단계 (특정 엔드포인트의 응답이 아님) |
...
> **인증 없이 없는 경로를 호출하면 404가 아니라 401입니다.** `/api/v1/**`는 인가 단계에서 먼저 걸러지며,
> 이는 토큰 없이 경로 존재 여부를 알아내지 못하게 하려는 의도입니다. 유효한 토큰이 있으면 기존대로 404가 나옵니다.
...
| `INVALID_SEASON_PERIOD` | 422 | 시즌 시작 시각은 종료 시각보다 앞서야 합니다. |
